...
async updateServerResources(serverId, newLimits) {
  try {
    // First, get the current server configuration to preserve existing values
    ...
    if (!currentServerResponse.ok) {
      console.error('[BOOST] Failed to fetch current server config:', await currentServerResponse.text());
      return false;
    }
    
    const currentServer = await currentServerResponse.json();
    const currentAttributes = currentServer.attributes;
    
    // Call the Pterodactyl API to update server resources with all required fields
    const response = await fetch(
      `${settings.pterodactyl.domain}/api/application/servers/${serverId}/build`,
      {
        ...
        body: JSON.stringify({
          allocation: currentAttributes.allocation, // Required: keep current allocation
          memory: newLimits.memory,
          swap: currentAttributes.limits.swap || 0, // Preserve current swap or default to 0
          disk: newLimits.disk,
          io: currentAttributes.limits.io || 500, // Preserve current IO or default to 500
          cpu: newLimits.cpu,
          threads: currentAttributes.limits.threads || null, // Preserve current threads
          feature_limits: {
            databases: currentAttributes.feature_limits.databases, // Required: preserve current database limit
            backups: currentAttributes.feature_limits.backups, // Required: preserve current backup limit
            allocations: currentAttributes.feature_limits.allocations || 0 // Preserve current allocation limit
          }
        })
      }
    );
    ...
    if (!response.ok) {
      ...
      console.error('[BOOST] Failed to update server resources:', errorText);
      return false;
    }
    
    console.log(`[BOOST] Successfully updated server ${serverId} resources:`, {
      memory: newLimits.memory,
      cpu: newLimits.cpu,
      disk: newLimits.disk
    });
